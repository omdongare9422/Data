...
void boundaryFill(GLint x, GLint y, Color fillColor, Color boundaryColor) {
    ...
    if ((current.r != boundaryColor.r || current.g != boundaryColor.g || current.b != boundaryColor.b) &&
        (current.r != fillColor.r || current.g != fillColor.g || current.b != fillColor.b)) {
...
void myMouse(int button,int state,int x,int y)
{
...

	{
	  ...
	  else if(pt==3)
	  {
		...
		Color boundaryColor = {1.0f, 1.0f, 0.0f};  // Yellow polygon border
		boundaryFill(x, Height - y, newColor, boundaryColor);
		pt=4;
	  }
	}
	glFlush();
